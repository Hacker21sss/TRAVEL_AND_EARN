...
const Driver = require('../../traveller/model/traveller'); // Assuming you have a Driver model
...
exports.getTravel = async (req, res) => {
  ...
  try {
    // Find travel records matching the locations
    const travelRecords = await Traveldetails.find({
      Leavinglocation,
      Goinglocation,
    });

    if (!travelRecords || travelRecords.length === 0) {
      return res.status(404).json({ message: 'No travel records found for the specified locations' });
    }

    // Find available drivers (assuming drivers have a status field)
    const availableDrivers = await Driver.find({ status: 'available' });

    // Return travel records and available drivers
    return res.status(200).json({
    ...
    console.error('Error retrieving travel details:', error.message);
    return res.status(500).json({ message: 'Internal server error', error: error.message });
  }
};